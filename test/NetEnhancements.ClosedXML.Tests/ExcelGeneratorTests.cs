...
namespace NetEnhancements.ClosedXML.Tests
{
	public class ExcelGeneratorTests
	{
		...
		public void AdjustToContents_On_Workbook_Adjusts_All_Columns_On_All_Sheets()
		{
			...
			var dataList = new List<MyClass>
							   {
								   new() { Prop1 = "AAAAAAAAAAAAA", Prop2 = 1 },
								   new() { Prop1 = "B", Prop2 = 2 },
								   new() { Prop1 = "C", Prop2 = 3 }
							   };
			...
			// Act
			var workbook = ExcelGenerator.GenerateExcel(dataList);
			workbook.AddAndPopulateSheet(dataList);

			var width = workbook.Worksheets.SelectMany(ws => ws.Columns().Select(c => c.Width)).ToList();

			workbook.AdjustToContents();

			var adjustedWidth = workbook.Worksheets.SelectMany(ws => ws.Columns().Select(c => c.Width)).ToList();
			...
			{
				var w = width[index];
				var aw = adjustedWidth[index];
				Assert.That(w, Is.Not.EqualTo(aw));
			}
		}
